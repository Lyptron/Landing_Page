...
function roadmapStatusStyle(status: string) {
  if (status === 'done') return 'bg-(--cp-emerald-soft) text-(--cp-emerald) border-(--cp-emerald-border)'
  if (status === 'in_progress') return 'bg-(--cp-cyan-soft) text-(--cp-cyan) border-(--cp-cyan-border)'
  return 'bg-(--cp-surface-strong) text-(--cp-text-faint) border-(--cp-border-soft)'
}
...
export default function FounderHub() {
  ...
  const alerts = useMemo(() => {
    ...
    overdueInvoices.forEach(i => items.push({ icon: AlertTriangle, tone: 'text-(--cp-red)', text: `Invoice ${i.invoice_number || i.id?.slice(0, 8)} for ${i.client_name} is overdue (₹${(i.amount || 0).toLocaleString('en-IN')})` }))
    atRiskProjects.forEach(p => items.push({ icon: AlertTriangle, tone: 'text-(--cp-amber)', text: `Project "${p.name}" is ${p.health === 'delayed' ? 'delayed' : 'at risk'}` }))
    expiringSubs.forEach(s => items.push({ icon: Clock, tone: 'text-(--cp-cyan)', text: `${s.name} renews in ${daysUntil(s.renewal_date)}d (₹${Number(s.monthly_cost || 0).toLocaleString('en-IN')}/mo)` }))
    return items
  ...
  return (
    <>
      <div className="mb-6">
        <h1 className="font-display text-2xl md:text-3xl font-bold tracking-tight text-(--cp-text)">Founder Hub</h1>
        <p className="text-(--cp-text-faint) text-[13px] mt-0.5">High-level financial overview and agency control center.</p>
      </div>

      {/* KPI Row */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {[
          { label: 'Pipeline', value: `₹${finances.total.toLocaleString('en-IN')}`, color: 'text-(--cp-text)' },
          { label: 'Received', value: `₹${finances.received.toLocaleString('en-IN')}`, color: 'text-(--cp-emerald)' },
          { label: 'Outstanding', value: `₹${finances.outstanding.toLocaleString('en-IN')}`, color: 'text-(--cp-amber)' },
          { label: 'Total Projects', value: String(projects.length), color: 'text-(--cp-text)' },
        ].map((kpi, i) => (
          <motion.div
            key={i}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5 mb-5">
        <div className="xl:col-span-2 p-5 rounded-2xl" style={{ background: 'var(--cp-surface)', border: '1px solid var(--cp-border-soft)' }}>
          <h3 className="text-[13px] font-bold text-(--cp-text-secondary) mb-4">Revenue Growth</h3>
        ...
        <div className="xl:col-span-1 flex flex-col rounded-2xl overflow-hidden" style={{ background: 'var(--cp-surface)', border: '1px solid var(--cp-border-soft)' }}>
          <div className="flex justify-between items-center px-5 py-4 border-b border-(--cp-border-soft)">
            <h3 className="text-[13px] font-bold text-(--cp-text-secondary)">Alerts</h3>
            <span className="text-[9px] font-mono text-(--cp-text-faint)">{alerts.length} active</span>
          </div>
          <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2 max-h-70">
            {loading ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center py-10">
                <CheckCircle2 className="w-8 h-8 text-(--cp-text-faint) mb-2" />
                <p className="text-[12px] text-(--cp-text-faint)">All clear. No alerts.</p>
              </div>
            ) : alerts.map((a, i) => (
              <div key={i} className="flex items-start gap-2.5 p-3 rounded-xl" style={{ border: '1px solid var(--cp-border-soft)' }}>
                ...
                <p className="text-[12px] text-(--cp-text-secondary) leading-snug">{a.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5 mb-5">
        ...
        <div className="xl:col-span-1 flex flex-col rounded-2xl overflow-hidden" style={{ background: 'var(--cp-surface)', border: '1px solid var(--cp-border-soft)' }}>
          <div className="flex justify-between items-center px-5 py-4 border-b border-(--cp-border-soft)">
            <h3 className="text-[13px] font-bold text-(--cp-text-secondary)">Pending Approvals</h3>
            <span className="text-[9px] font-mono text-(--cp-text-faint)">{approvals.length}</span>
          </div>
          <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2 max-h-70">
            {approvals.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center py-10">
                <ShieldCheck className="w-8 h-8 text-(--cp-text-faint) mb-2" />
                <p className="text-[12px] text-(--cp-text-faint)">Nothing pending approval.</p>
              </div>
            ) : approvals.map((a) => (
              <div key={a.id} className="p-3 rounded-xl" style={{ border: '1px solid var(--cp-border-soft)' }}>
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleApproval(a.id, 'approved')} className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider text-(--cp-emerald) bg-(--cp-emerald-soft) border border-(--cp-emerald-border) hover:bg-(--cp-emerald-soft) transition-colors">
                    <CheckCircle2 className="w-3 h-3" /> Approve
                  </button>
                  <button onClick={() => handleApproval(a.id, 'rejected')} className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider text-(--cp-red) bg-(--cp-red-soft) border border-(--cp-red-border) hover:bg-(--cp-red-soft) transition-colors">
        ...
        <div className="xl:col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-3">
          {[
            ...
            <Link key={i} href={dept.href} className="p-4 rounded-2xl flex flex-col justify-between hover:bg-(--cp-surface-strong) transition-colors group" style={{ background: 'var(--cp-surface)', border: '1px solid var(--cp-border-soft)' }}>
              <div className="flex items-center justify-between mb-4">
                <dept.icon className="w-4 h-4 text-(--cp-text-faint)" />
                <ArrowRight className="w-3.5 h-3.5 text-(--cp-text-faint) group-hover:text-(--cp-text-muted) group-hover:translate-x-0.5 transition-all" />
              </div>
              <div>
                <p className="text-[20px] font-display font-bold text-(--cp-text) tracking-tight">{dept.primary}</p>
                <p className="text-[10px] text-(--cp-text-faint) mt-0.5">{dept.sub}</p>
                <p className="text-[9px] font-mono uppercase tracking-[0.15em] text-(--cp-text-faint) mt-2">{dept.label}</p>
              </div>
            </Link>
          ))}

          {/* Subscription Snapshot */}
          <Link href="/admin/subscriptions" className="sm:col-span-3 p-4 rounded-2xl flex items-center justify-between hover:bg-(--cp-surface-strong) transition-colors group" style={{ background: 'var(--cp-surface)', border: '1px solid var(--cp-border-soft)' }}>
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'var(--cp-surface-strong)' }}>
                <CreditCard className="w-4 h-4 text-(--cp-text-muted)" />
              </div>
              <div>
                <p className="text-[12px] font-semibold text-(--cp-text-secondary)">Subscription Spend</p>
                <p className="text-[10px] text-(--cp-text-faint) mt-0.5">{subTotals.renewalsSoon} renewal{subTotals.renewalsSoon === 1 ? '' : 's'} due in next 30 days</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <p className="text-[18px] font-display font-bold text-(--cp-text) tracking-tight">₹{subTotals.monthly.toLocaleString('en-IN')}<span className="text-[10px] text-(--cp-text-faint) font-sans">/mo</span></p>
              <ArrowRight className="w-3.5 h-3.5 text-(--cp-text-faint) group-hover:text-(--cp-text-muted) group-hover:translate-x-0.5 transition-all" />
            </div>
          </Link>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--cp-surface)', border: '1px solid var(--cp-border-soft)' }}>
          <div className="flex justify-between items-center px-5 py-4 border-b border-(--cp-border-soft)">
            <div className="flex items-center gap-2">
              <Rocket className="w-3.5 h-3.5 text-(--cp-text-faint)" />
              <h3 className="text-[13px] font-bold text-(--cp-text-secondary)">Growth Roadmap</h3>
            </div>
            <button onClick={openRoadmapModal} className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-semibold text-(--cp-text-muted) hover:text-(--cp-text) transition-all" style={{ background: 'var(--cp-surface-strong)', border: '1px solid var(--cp-border)' }}>
              <Plus className="w-3 h-3" /> Add
            </button>
          </div>
          <div className="p-3 flex flex-col gap-2 max-h-65 overflow-y-auto">
            {roadmapList.length === 0 ? (
              <p className="text-[12px] text-(--cp-text-faint) text-center py-8">No roadmap items yet.</p>
              ...
              <div key={item.id} className="flex items-center justify-between p-3 rounded-xl group" style={{ border: '1px solid var(--cp-border-soft)' }}>
                <div className="min-w-0">
                  <p className="text-[12px] font-medium text-(--cp-text-secondary) truncate">{item.title}</p>
        ...
        <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--cp-surface)', border: '1px solid var(--cp-border-soft)' }}>
          <div className="flex justify-between items-center px-5 py-4 border-b border-(--cp-border-soft)">
            <div className="flex items-center gap-2">
              <Users2 className="w-3.5 h-3.5 text-(--cp-text-faint)" />
              <h3 className="text-[13px] font-bold text-(--cp-text-secondary)">Hiring Plan</h3>
            </div>
            <button onClick={() => { setRoadmapForm({ ...emptyRoadmapForm, type: 'hiring' }); setRoadmapModalOpen(true) }} className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-semibold text-(--cp-text-muted) hover:text-(--cp-text) transition-all" style={{ background: 'var(--cp-surface-strong)', border: '1px solid var(--cp-border)' }}>
              <Plus className="w-3 h-3" /> Add
            </button>
          </div>
          <div className="p-3 flex flex-col gap-2 max-h-65 overflow-y-auto">
            {hiringList.length === 0 ? (
              <p className="text-[12px] text-(--cp-text-faint) text-center py-8">No hiring plans yet.</p>
            ) : hiringList.map((item) => (
              <div key={item.id} className="flex items-center justify-between p-3 rounded-xl group" style={{ border: '1px solid var(--cp-border-soft)' }}>
                <div className="min-w-0">
                  <p className="text-[12px] font-medium text-(--cp-text-secondary) truncate">{item.title}</p>
      ...
      <Modal open={roadmapModalOpen} onClose={() => setRoadmapModalOpen(false)} title="Add Plan Item" subtitle="Add a growth roadmap or hiring plan item.">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-(--cp-border-soft)">
            <button onClick={() => setRoadmapModalOpen(false)} className="px-4 py-2 rounded-xl text-[12px] font-medium text-(--cp-text-faint) hover:text-(--cp-text-secondary) transition-colors">Cancel</button>
            <button onClick={saveRoadmapItem} disabled={savingRoadmap || !roadmapForm.title} className="px-5 py-2 bg-(--cp-cyan) text-white font-semibold text-[12px] rounded-xl hover:bg-(--cp-cyan-strong) transition-all disabled:opacity-30">
