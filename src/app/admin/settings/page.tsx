...
export default function SettingsPage() {
  ...
  async function handleLogoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (error || !url) {
      setLogoError(error?.message || "Upload failed. Try a smaller file or different format.")
  ...
  async function handleSaveProfile() {
    ...
    if (error) {
      setProfileError(error.message || "Couldn't save profile. Try again.")
    } else {
  ...
  async function handleSaveNotifications() {
    ...
    if (error) {
      setNotificationsError("Couldn't save notification preferences. Try again.")
    } else {
      ...
      <div className="flex flex-col md:flex-row gap-8 lg:gap-12">
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-[13px] font-medium transition-all ${
                  isActive
                    ? isDanger
                      ? 'bg-(--cp-red-soft) text-(--cp-red) border border-(--cp-red-border)'
                      : 'bg-(--cp-surface-strong) text-(--cp-text) border border-(--cp-border)'
                    : isDanger
                      ? 'text-(--cp-red) hover:bg-(--cp-red-soft) border border-transparent opacity-80 hover:opacity-100'
                      : 'text-(--cp-text-faint) hover:bg-(--cp-surface) border border-transparent'
                }`}
              >
        ...
        <div className="flex-1 max-w-3xl">
            ...
            >
              <div className="premium-card p-6 md:p-8 rounded-3xl shadow-2xl relative overflow-hidden">
                <div className="mb-6 border-b border-(--cp-border-soft) pb-6">
                  <h2 className="font-display text-xl font-bold text-(--cp-text)">Agency Profile</h2>
                  <p className="text-[13px] text-(--cp-text-faint) mt-1">This is your agency&apos;s primary identity across the platform.</p>
                </div>
                ...
                <div className="flex flex-col gap-6">
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <label className="px-4 py-2 bg-(--cp-surface) hover:bg-(--cp-surface-strong) border border-(--cp-border-soft) rounded-lg text-[13px] text-(--cp-text) transition-colors mb-2 cursor-pointer inline-block">
                      ...
                      {logoUrl && (
                        <button
                          ...
                          className="text-[11px] text-(--cp-red) opacity-60 hover:opacity-100 mt-1 transition-opacity"
                        >
                          Remove logo
                        </button>
                      )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="flex flex-col gap-2">
                      <label className="text-[11px] font-semibold uppercase tracking-wider text-(--cp-text-muted)">Agency Name</label>
                      <input
                        type="text"
                        value={agencyName}
                        onChange={(e) => setAgencyName(e.target.value)}
                        className="px-4 py-2.5 bg-(--cp-bg-soft) border border-(--cp-border) rounded-lg text-[13px] text-(--cp-text) focus:outline-none focus:border-(--cp-border-strong) transition-colors"
                      />
                    </div>
                    <div className="flex flex-col gap-2">
                      <label className="text-[11px] font-semibold uppercase tracking-wider text-(--cp-text-muted)">Contact Email</label>
                      <input
                        type="email"
                        value={agencyEmail}
                    ...
                    <div className="flex flex-col gap-2 md:col-span-2">
                      <label className="text-[11px] font-semibold uppercase tracking-wider text-(--cp-text-muted)">Website</label>
                      <input
                        type="text"
                        value={agencyWebsite}
                ...
                <div className="mt-8 pt-6 border-t border-(--cp-border-soft) flex justify-end">
                  <button
                    onClick={handleSaveGeneral}
                    disabled={savingGeneral}
                    className="px-6 py-2.5 bg-(--cp-cyan) text-white font-semibold text-[13px] rounded-full hover:bg-(--cp-cyan-strong) transition-all shadow-[0_0_20px_color-mix(in_srgb,var(--cp-cyan)_30%,transparent)] disabled:opacity-50"
                  >
          ...
          {activeTab === 'access-codes' && (
            ...
            >
              <div className="premium-card p-6 md:p-8 rounded-3xl shadow-2xl relative overflow-hidden">
                <div className="mb-6 border-b border-(--cp-border-soft) pb-6 flex items-start justify-between">
                  <div>
                    <h2 className="font-display text-xl font-bold text-(--cp-text)">Client Access Codes</h2>
                    <p className="text-[13px] text-(--cp-text-faint) mt-1">Generate, manage, and revoke access codes for client portals. Clients use these to log into their dashboard.</p>
                  </div>
                  <button onClick={loadProjects} className="p-2 rounded-lg text-(--cp-text-faint) hover:text-(--cp-text-secondary) hover:bg-(--cp-surface-strong) transition-all" title="Refresh">
                ...
                {projects.length === 0 && !loading ? (
                  <div className="text-center py-12">
                    <Key className="w-8 h-8 text-(--cp-text-faint) mx-auto mb-3" />
                    <p className="text-(--cp-text-faint) text-[13px]">No projects found. Create a project first from the Projects page.</p>
                  </div>
                ) : (
                  <div className="flex flex-col gap-4">
                    {projects.map((project) => (
                      <div
                        key={project.id}
                        className="p-5 rounded-2xl border border-(--cp-border-soft) hover:border-(--cp-border) transition-all"
                        style={{ background: 'var(--cp-surface-strong)' }}
                      >
                        <div className="flex items-start justify-between gap-4 mb-3">
                          <div>
                            <h3 className="font-bold text-(--cp-text) text-[15px]">{project.name}</h3>
                            <p className="text-[11px] text-(--cp-text-faint) font-mono mt-0.5">{project.client_email || 'No client email'}</p>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className={`px-2 py-0.5 rounded-full text-[9px] font-mono font-bold uppercase tracking-wider ${
                              project.status === 'completed' ? 'bg-(--cp-emerald-soft) text-(--cp-emerald) border border-(--cp-emerald-border)' :
                              project.status === 'in-progress' ? 'bg-(--cp-cyan-soft) text-(--cp-cyan) border border-(--cp-cyan-border)' :
                              'bg-(--cp-surface) text-(--cp-text-faint) border border-(--cp-border-soft)'
                            }`}>
                              {project.status}
                            </span>
                          </div>
                        ...
                        {project.access_code ? (
                          <div className="flex items-center gap-3 mt-4">
                            <div className="flex-1 flex items-center gap-3 px-4 py-3 rounded-xl" style={{ background: 'var(--cp-cyan-soft)', border: '1px solid var(--cp-cyan-border)' }}>
                              <Key className="w-4 h-4 text-(--cp-cyan) opacity-60 shrink-0" />
                              <span className="font-mono text-[15px] font-bold text-(--cp-cyan) tracking-[0.2em]">{project.access_code}</span>
                            </div>
                            <button
                              onClick={() => copyToClipboard(project.access_code, project.id)}
                              className="p-3 rounded-xl text-(--cp-text-faint) hover:text-(--cp-text-secondary) hover:bg-(--cp-surface-strong) transition-all"
                              title="Copy code"
                            >
                              {copiedId === project.id ? <CheckCircle2 className="w-4 h-4 text-(--cp-emerald)" /> : <Copy className="w-4 h-4" />}
                            </button>
                            <button
                            ...
                            >
                              {copiedId === project.id + '-url' ? <CheckCircle2 className="w-4 h-4 text-(--cp-emerald)" /> : <ExternalLink className="w-4 h-4" />}
                            </button>
                            <button
                              onClick={() => handleRemoveCode(project.id)}
                              className="p-3 rounded-xl text-(--cp-text-faint) hover:text-(--cp-red) hover:bg-(--cp-red-soft) transition-all"
                              title="Revoke code"
                            >
                          ...
                          <div className="flex items-center gap-3 mt-4">
                            <input
                              ...
                              className="flex-1 px-4 py-3 rounded-xl text-[13px] text-(--cp-text) font-mono tracking-widest outline-none"
                            ...
                            <button
                              ...
                              className="px-5 py-3 bg-(--cp-cyan) text-white font-semibold text-[12px] rounded-xl transition-all hover:bg-(--cp-cyan-strong)"
                          ...
                          <button
                            ...
                            className="flex items-center gap-2 mt-4 px-4 py-2.5 rounded-xl text-[12px] font-semibold text-(--cp-text-muted) hover:text-(--cp-text-secondary) hover:bg-(--cp-surface-strong) transition-all border border-dashed border-(--cp-border)"
                          >
              ...
              <div className="premium-card p-6 rounded-3xl relative overflow-hidden">
                <div className="flex items-start gap-4 relative z-10">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background: 'var(--cp-cyan-soft)', border: '1px solid var(--cp-cyan-border)' }}>
                    <Settings2 className="w-5 h-5 text-(--cp-cyan)" />
                  </div>
                  <div>
                    <h3 className="font-bold text-(--cp-text) text-[14px] mb-1">How Client Codes Work</h3>
                    <p className="text-[12px] text-(--cp-text-faint) leading-relaxed">
                      Each project can have a unique access code. Share the code with your client — they enter it at <span className="font-mono text-(--cp-cyan) opacity-60">/client</span> to access their project dashboard. You can revoke or regenerate codes anytime. Everything the client sees is pulled from your Supabase data in real-time.
                    </p>
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'profile' && (
            ...
            >
              <div className="premium-card p-6 md:p-8 rounded-3xl shadow-2xl relative overflow-hidden">
                <div className="mb-6 border-b border-(--cp-border-soft) pb-6">
                  <h2 className="font-display text-xl font-bold text-(--cp-text)">My Profile</h2>
                  <p className="text-[13px] text-(--cp-text-faint) mt-1">Your admin account details.</p>
                </div>
                ...
                <div className="flex items-center gap-6 mb-8">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-(--cp-text)">{user?.name || 'Admin'}</h3>
                    <p className="text-[13px] text-(--cp-text-faint)">{user?.email}</p>
                    <p className="text-[9px] font-mono uppercase tracking-[0.15em] text-(--cp-emerald) opacity-60 mt-1">Authenticated</p>
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex flex-col gap-2">
                    <label className="text-[11px] font-semibold uppercase tracking-wider text-(--cp-text-muted)">Display Name</label>
                    <input
                      type="text"
                      value={displayName}
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="text-[11px] font-semibold uppercase tracking-wider text-(--cp-text-muted)">Email</label>
                    <input
                      type="email"
                      defaultValue={user?.email || ''}
                      disabled
                      className="px-4 py-2.5 bg-(--cp-bg-soft) border border-(--cp-border) rounded-lg text-[13px] text-(--cp-text-muted) cursor-not-allowed"
                    />
                  </div>
                </div>

                <div className="mt-8 pt-6 border-t border-(--cp-border-soft) flex items-center justify-end gap-3">
                  {profileError && (
                    <span className="text-[12px] text-(--cp-red)" role="alert">{profileError}</span>
                  )}
                  {profileSaved && (
                    <span className="flex items-center gap-1.5 text-[12px] text-(--cp-emerald)">
          ...
          {activeTab === 'integrations' && (
            ...
            >
              <div className="premium-card p-6 md:p-8 rounded-3xl shadow-2xl relative overflow-hidden">
                <div className="mb-6 border-b border-(--cp-border-soft) pb-6">
                  <h2 className="font-display text-xl font-bold text-(--cp-text)">Installed Integrations</h2>
                  <p className="text-[13px] text-(--cp-text-faint) mt-1">Connect your agency&apos;s tools to automate your workflow.</p>
                </div>

                <div className="flex flex-col gap-4">
                  {/* Supabase — Connected */}
                  <div className="flex items-center justify-between p-4 bg-(--cp-surface-strong) border border-(--cp-border-soft) rounded-xl hover:border-(--cp-border) transition-colors">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="text-[13px] font-bold text-(--cp-text) flex items-center gap-2">
                          Supabase <CheckCircle2 className="w-3 h-3 text-(--cp-emerald)" />
                        </h4>
                        <p className="text-[11px] text-(--cp-text-faint)">Database & Auth connected</p>
                      </div>
                    </div>
                    <span className="px-3 py-1.5 bg-(--cp-emerald-soft) border border-(--cp-emerald-border) text-(--cp-emerald) text-[9px] font-bold font-mono uppercase tracking-[0.15em] rounded-full">Active</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-4 bg-(--cp-surface-strong) border border-(--cp-border-soft) rounded-xl hover:border-(--cp-border) transition-colors">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="text-[13px] font-bold text-(--cp-text) flex items-center gap-2">
                          Google OAuth {user?.avatar_url && <CheckCircle2 className="w-3 h-3 text-(--cp-emerald)" />}
                        </h4>
                        <p className="text-[11px] text-(--cp-text-faint)">{user?.avatar_url ? 'Sign-in with Google enabled' : 'Enable Google sign-in for admins'}</p>
                      </div>
                    </div>
                    <span className={`px-3 py-1.5 rounded-full text-[9px] font-bold font-mono uppercase tracking-[0.15em] ${user?.avatar_url ? 'bg-(--cp-emerald-soft) border border-(--cp-emerald-border) text-(--cp-emerald)' : 'bg-(--cp-surface) border border-(--cp-border-soft) text-(--cp-text-faint)'}`}>
                  ...
                  <div className="flex items-center justify-between p-4 bg-(--cp-surface-strong) border border-(--cp-border-soft) rounded-xl hover:border-(--cp-border) transition-colors">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="text-[13px] font-bold text-(--cp-text)">GitHub</h4>
                        <p className="text-[11px] text-(--cp-text-faint)">Connect for commit tracking</p>
                      </div>
                    </div>
                    <button className="px-4 py-1.5 bg-(--cp-cyan) text-white font-semibold rounded-full text-[11px] transition-colors shadow-[0_0_20px_color-mix(in_srgb,var(--cp-cyan)_30%,transparent)]">
                      Connect
                    </button>
                  </div>
                  ...
                  {isFounder && (
                    <div className="flex items-center justify-between p-4 bg-(--cp-surface-strong) border border-(--cp-border-soft) rounded-xl hover:border-(--cp-border) transition-colors">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h4 className="text-[13px] font-bold text-(--cp-text)">Stripe</h4>
                          <p className="text-[11px] text-(--cp-text-faint)">Sync invoices and payments</p>
                        </div>
                      </div>
                      <button className="px-4 py-1.5 bg-(--cp-cyan) text-white font-semibold rounded-full text-[11px] transition-colors shadow-[0_0_20px_color-mix(in_srgb,var(--cp-cyan)_30%,transparent)]">
                        Connect
          ...
          {activeTab === 'notifications' && (
            ...
            >
              <div className="mb-6 border-b border-(--cp-border-soft) pb-6">
                <h2 className="font-display text-xl font-bold text-(--cp-text)">Notification Preferences</h2>
                <p className="text-[13px] text-(--cp-text-faint) mt-1">Choose what notifications you want to receive.</p>
              </div>

              <div className="flex flex-col gap-5">
                {NOTIFICATION_ITEMS.map((item) => (
                  <div key={item.key} className="flex items-center justify-between p-4 bg-(--cp-surface-strong) border border-(--cp-border-soft) rounded-xl">
                    <div>
                      <h4 className="text-[13px] font-medium text-(--cp-text)">{item.label}</h4>
                      <p className="text-[11px] text-(--cp-text-faint) mt-0.5">{item.desc}</p>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer">
                      ...
                      />
                      <div className="w-10 h-5 bg-(--cp-border) peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-(--cp-bg) after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:bg-(--cp-text-muted) after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-[color-mix(in_srgb,var(--cp-cyan)_30%,transparent)] peer-checked:after:bg-(--cp-cyan)" />
                    </label>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-8 pt-6 border-t border-(--cp-border-soft) flex items-center justify-end gap-3">
                {notificationsError && (
                  <span className="text-[12px] text-(--cp-red)" role="alert">{notificationsError}</span>
                )}
                {notificationsSaved && (
                  <span className="flex items-center gap-1.5 text-[12px] text-(--cp-emerald)">
                    <CheckCircle2 className="w-3.5 h-3.5" /> Saved
          ...
          {activeTab === 'security' && (
            ...
            >
              <div className="mb-6 border-b border-(--cp-border-soft) pb-6">
                <h2 className="font-display text-xl font-bold text-(--cp-text)">Security</h2>
                <p className="text-[13px] text-(--cp-text-faint) mt-1">Manage your authentication and security settings.</p>
              </div>

              <div className="flex flex-col gap-6">
                <div className="p-5 rounded-2xl border border-(--cp-border-soft)" style={{ background: 'var(--cp-surface-strong)' }}>
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <h4 className="text-[14px] font-bold text-(--cp-text)">Authentication Method</h4>
                      <p className="text-[12px] text-(--cp-text-faint) mt-0.5">How you sign in to the admin panel</p>
                    </div>
                    <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-(--cp-emerald-soft) border border-(--cp-emerald-border)">
                      <CheckCircle2 className="w-3 h-3 text-(--cp-emerald)" />
                      <span className="text-(--cp-emerald) text-[9px] font-bold font-mono uppercase tracking-[0.15em]">Active</span>
                    </div>
                  </div>
                  <p className="text-[13px] text-(--cp-text-secondary)">
                    Signed in as <span className="font-mono text-(--cp-cyan) opacity-70">{user?.email}</span>
                    {user?.avatar_url ? ' via Google OAuth' : ' via email/password'}
                  </p>
                </div>

                <div className="p-5 rounded-2xl border border-(--cp-border-soft)" style={{ background: 'var(--cp-surface-strong)' }}>
                  <h4 className="text-[14px] font-bold text-(--cp-text) mb-2">Row Level Security</h4>
                  <p className="text-[12px] text-(--cp-text-faint) leading-relaxed">
          ...
          {activeTab === 'danger' && (
            ...
            >
              ...
              >
                <div className="flex items-start gap-4">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background: 'color-mix(in srgb, var(--cp-red) 18%, transparent)', border: '1px solid var(--cp-red-border)' }}>
                    <AlertTriangle className="w-5 h-5 text-(--cp-red)" />
                  </div>
                  <div>
                    <h2 className="font-display text-xl font-bold text-(--cp-text)">Danger Zone</h2>
                    <p className="text-[13px] text-(--cp-text-secondary) mt-1 leading-relaxed">
              ...
              {dangerDone && (
                ...
                >
                  <CheckCircle2 className="w-4 h-4 text-(--cp-emerald) shrink-0" />
                  <p className="text-[13px] text-(--cp-text-secondary)">{dangerDone}</p>
                </div>
              )}
              ...
              {(
                ...
                >
                  <div className="flex items-start gap-3 min-w-0">
                    ...
                    >
                      <item.icon className={`w-4 h-4 ${item.severe ? 'text-(--cp-red)' : 'text-(--cp-text-muted)'}`} />
                    </div>
                    <div className="min-w-0">
                      <h4 className="text-[14px] font-bold text-(--cp-text)">{item.label}</h4>
                      <p className="text-[12px] text-(--cp-text-faint) mt-1 leading-relaxed">{item.desc}</p>
                    </div>
                  </div>
                  <button
